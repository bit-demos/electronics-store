# Electronic Store App

The Electronics store is built as an app using the app aspect of Bit. The app is made up of components from the base-ui scope and the e-commerce scope.

Github: [electronics store repo](https://github.com/bit-demos/electronics-store)

## The electronics-store scope contains the following:

### apps

This is the main app aspect. The `app.tsx` file contains the main application logic including the `<Head>` tags and the Router. It is in this file that you would add fonts to your head tag as well as configure meta tags. You can also use your Theme component here to wrap everything in the theme provider.

## Entity

An array of mock data to show some products according to the data structure. This data strucuture extends the e-commerce entity and adds the extra types of available colors to the data structure.

## UI Components

UI components that could be used to create a store's UI. These include but are not limited to:

- A pages folder with pages for the home page and about page. These page components are then used inside the main app.
